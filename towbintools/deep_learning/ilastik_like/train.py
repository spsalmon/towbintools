...
def preprocess_ground_truth_mask(ground_truth):
    ...
    dilated_ground_truth = (
        cv2.morphologyEx(ground_truth, cv2.MORPH_DILATE, kernel) > 0
    ).astype(int)
    background = (dilated_ground_truth == 0).astype(int)
    ground_truth = (cv2.morphologyEx(ground_truth, cv2.MORPH_ERODE, kernel) > 0).astype(
        int
    )
...
def extract_vgg16_pyramid_features(model, x, ratios):
    ...
    for shape in shapes:
        rescaled_batch = F.interpolate(
            x, size=shape, mode="bilinear", align_corners=True
        )
        scaled_features = model(rescaled_batch)
        rescaled_features = F.interpolate(
            scaled_features, size=x.shape[2:], mode="bilinear", align_corners=False
        )
...
images = [
    os.path.join(image_dir, f) for f in os.listdir(image_dir) if f.endswith(".tiff")
]
random.seed(42)
...
test_loader = DataLoader(
    IlastikLikeTrainingDataset(
        training_images, training_ground_truth, transform=transform_img
    ),
    batch_size=1,
    shuffle=False,
    num_workers=16,
...
predictions = Parallel(n_jobs=2, prefer="processes")(
    delayed(process_image_for_training)(img, mask, model, ratios)
